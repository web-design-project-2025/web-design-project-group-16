...
function toggleHamburgerNav() {
  const menu = document.getElementById("hamburgermenu-id");
...
window.addEventListener("resize", () => {
  const menu = document.getElementById("hamburgermenu-id");
  if (window.innerWidth > 800) {
    menu.style.display = "block"; // Ensure it's visible on wide screens
  } else {
    menu.style.display = "none"; // Reset to hidden on small screens
  }
});
// End help chatGPT
